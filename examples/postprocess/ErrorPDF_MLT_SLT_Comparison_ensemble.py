...
for ivar in range(3):
    ...
    for isub in range(len(variable_case_dic[var])):
        ...
        pdf_runs_uni = []
        for irun in range(1,5):
            pdf_runs_uni.append(np.interp(xnew,xcen_runs[irun-1],pdf_runs[irun-1]))
        ...
        ax.plot(xnew,pdf_runs_mean,linelib[il],color=colorlib[ic],linewidth=2.0,label=line_labels[ic])
        ax.fill_between(xnew, pdf_runs_mean - pdf_runs_std, pdf_runs_mean + pdf_runs_std, facecolor=colorlib[ic], interpolate=True, alpha=.4)
        ax.plot([0, 0], [0, 55], "k--")
    if icol == 0:
