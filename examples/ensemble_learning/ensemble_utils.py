...
def test_ens(model_ens, loader, verbosity, num_samples=None, saveresultsto=None):
    ...
    for data in iterate_tqdm(loader, verbosity):
        ...
        for imodel, pred in enumerate(pred_ens):
            ...
            for ihead in range(num_heads):
                if debug_nan(pred[ihead], message="pred from model %d"%imodel):
                    print("Pei debugging: NAN detected in prediction of model %s"%model_ens.module.model_dir_list[imodel])
...
def test_ens_GFM(model_ens, loader, verbosity, num_samples=None, saveresultsto=None):
    ...
    for data in iterate_tqdm(loader, verbosity):
        ...
        ###########################
        #print(data.x.size())
        x_atomnums.extend(data.x[:,0])
        graph_batch.extend(data.batch)
        ###########################
        pred_ens = model_ens(data)
        ###########################
        ytrue = data.y
    ...
    x_atomnums_tensor=torch.tensor(x_atomnums)
    graph_batch_tensor=torch.tensor(graph_batch)
    x_atomnums_tensor = gather_tensor_ranks(x_atomnums_tensor)
    graph_batch_tensor = gather_tensor_ranks(graph_batch_tensor)
    ...
    for ihead in range(num_heads):
        ...
        if saveresultsto is not None and hydragnn.utils.get_comm_size_and_rank()[1]==0:
            ...
            m = {'true': true_values[ihead], 'pred_ens': head_pred_ens, 'atomnums':x_atomnums_tensor, 'graph_batch': graph_batch_tensor}
            #torch.save(m, saveresultsto +"head%d_atomnum_batch_%s.db"%(ihead, str(device)))
            torch.save(m, saveresultsto +"head%d_atomnum_batch_%s.db"%(ihead, str(hydragnn.utils.get_comm_size_and_rank()[1])))
