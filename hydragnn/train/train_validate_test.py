...
def train(
    ...
    for ibatch, data in iterate_tqdm(
        ...
        print_distributed(verbosity, f"Memory used {torch.cuda.max_memory_allocated()/1e9} GB, before opt")
        ...
        if use_deepspeed:
            ...
            scaler.update()
            #opt.step()
        print_distributed(verbosity, f"Memory used {torch.cuda.max_memory_allocated()/1e9} GB, after opt")
...
@torch.no_grad()
def validate(loader, model, verbosity, reduce_ranks=True, compute_grad_energy=False, autocast_context=nullcontext()):
    total_error = torch.tensor(0.0, device=get_device())
    tasks_error = torch.zeros(model.module.num_heads, device=get_device())
    num_samples_local = 0
    model.eval()
